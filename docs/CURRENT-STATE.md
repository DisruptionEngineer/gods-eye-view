...
> as planned counts (slice 2 wires them up). No file means no endpoint traffic
> and no SITE POI; nothing in this feature is required to run the app.
>
...
> layers **3 and 9** — layer 3, not 4 — LOD gate ≤15 km, with degraded partial
> results if one of the two sub-fetches fails). All three gate on
> `camera.moveEnd`, never `camera.changed`/`percentageChanged` — `moveEnd` is
> the only event that means "the camera has actually settled," and a
> `_pendingRefresh` queue re-triggers a fetch if a `moveEnd` lands while a prior
> fetch for the same layer is still in flight, so a settle mid-fetch is never
> silently dropped. Wind does **not** use `createRetryableLoader` (it memoizes
> success permanently, which is wrong for a layer that must re-fetch on every
> camera settle) — it does a direct fetch with a 30 s failure cooldown instead.
>
